...
function createModule(
  moduleName: string,
  options: { mode?: 'compile-only' | 'extract-css' } = {}
) {
  const { mode = 'compile-only' } = options;

  return {
    moduleName,
    specifiers: {
      makeStyles: async ({ context, node, parent, params, utils }) => {
        const specifier = mode === 'compile-only' ? '__styles' : '__css';
        const importName = utils.addNamedImport(moduleName, specifier);
      ...
      makeResetStyles: async ({ context, node, parent, params, utils }) => {
        const specifier =
          mode === 'compile-only' ? '__resetStyles' : '__resetCSS';
        const importName = utils.addNamedImport(moduleName, specifier);
...
export function createModuleConfig(options: {
  ...
  return [
    createModule('@griffel/core', options),
    createModule('@griffel/react', options),
    createModule('@fluentui/react-components', options),
  ];
}
