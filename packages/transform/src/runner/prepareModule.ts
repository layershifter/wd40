...
export async function prepareModule(
  ...
  // }
  return prepareJavaScriptModule(code, url);
}
...
async function prepareJavaScriptModule(
  code: string,
  url: string
): Promise<any | null> {
  ...
  function getImportId(): string {
    return `__vite_ssr_import_${uid++}__`;
  }
  ...
  for (const node of ast.body as Node[]) {
    ...
    if (node.type === 'ImportDeclaration') {
      ...
      defineImport(importId, node.source.value as string, specifiers);
    ...
    if (node.type === 'ExportNamedDeclaration') {
      if (node.declaration) {
        ...
        if (node.source) {
          ...
          defineImport(importId, node.source.value as string);
        } else {
          // export { foo, bar }
          for (const spec of node.specifiers) {
