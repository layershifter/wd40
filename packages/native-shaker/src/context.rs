...
impl<'a> TransformContext<'a> {
  pub fn new(allocator: &'a Allocator, filename: &'a str, source_text: &'a str) -> Self {
  ...
  fn wrap_diagnostics<D: IntoIterator<Item = OxcDiagnostic>>(
    ...
    let source = {
      ...
      let ns =
        NamedSource::new(self.file_name(), self.source_text().to_string()).with_language(lang);
      Arc::new(ns)
    };
